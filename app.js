...
function showToast(msg, color, actionHtml = "") {
    ...
    toast.style.background = "rgba(15, 15, 25, 0.8)";
    toast.style.backdropFilter = "blur(20px)";
    toast.style.border = "1px solid rgba(255, 255, 255, 0.1)";
    toast.style.borderLeft = `4px solid ${color}`;

    let icon = "ph-info";
    if (color === "#e11d48") { color = "#f472b6"; icon = "ph-bold ph-warning-diamond"; } 
    else if (color === "#6366f1") { color = "#c084fc"; icon = "ph-bold ph-planet"; } 
    else if (color === "#0ea5e9") { color = "#38bdf8"; icon = "ph-bold ph-lightning"; } 
    else if (color === "#059669") { color = "#2dd4bf"; icon = "ph-bold ph-check-circle"; } 

    toast.innerHTML = `
        <i class="${icon}" style="font-size:1.5rem; color:${color}; flex-shrink:0;"></i>
        <div class="toast-msg">
            <div style="font-weight:900; color:#fff; font-size:0.7rem; letter-spacing:1px; text-transform:uppercase; margin-bottom:4px">Protocol Transmission</div>
            <div style="color:var(--text-muted); font-size:0.9rem; font-weight:500;">${msg}</div>
            ${actionHtml ? `<div style="margin-top:12px">${actionHtml}</div>` : ''}
        </div>
        <button class="toast-close" onclick="this.parentElement.remove()" style="color:rgba(255,255,255,0.4)"><i class="ph ph-x"></i></button>
    `;
    ...
    const fetchFn = async () => {
        try {
            ...
            data._embedded.records.forEach((r, i) => {
                ...
                const isNew = i % 2 === 0;
                const iconClass = isNew ? "ph-bold ph-plus-square" : "ph-bold ph-fingerprint";
                const label = isNew ? "PROPOSAL_CREATED" : "VOTE_SUBMITTED";
                const color = isNew ? "var(--primary)" : "#10b981";

                feed.innerHTML += `
                    <div class="event-item" style="border-left: 2px solid ${color}">
                        <div class="event-icon" style="color:${color}"><i class="${iconClass}"></i></div>
                        <div class="event-content">
                            <span class="event-type" style="color:${color}; font-size:0.6rem;">${label}</span>
                            <span class="event-addr">${pk.substring(0,8)}…${pk.slice(-4)}</span>
                            <span class="event-time" style="font-size:0.65rem; opacity:0.6">${time}</span>
                        </div>
                    </div>`;
            });
        } catch (e) {
...
function txStatus(state, hash = "") {
    ...
    bar.style.backdropFilter = "blur(30px)";

    if (state === "signing") {
        bar.style.borderColor = "var(--primary)";
        bar.innerHTML = "<i class=\"ph-bold ph-key\" style=\"color:var(--primary);\"></i> Authenticating with Cosmic Layer…";
    } else if (state === "submitting") {
        bar.style.borderColor = "var(--secondary)";
        bar.innerHTML = "<i class=\"ph-bold ph-planet\" style=\"color:var(--secondary); animation:spin 2s linear infinite;\"></i> Transmitting Proposal to Stellar…";
    } else if (state === "success") {
        bar.style.borderColor = "#2dd4bf";
        bar.innerHTML = `<i class="ph-bold ph-seal-check" style="color:#2dd4bf;"></i> Confirmed <a href="https://stellar.expert/explorer/testnet/tx/${hash}" target="_blank" style="color:var(--secondary); text-decoration:none; margin-left:15px; font-weight:800; text-transform:uppercase; font-size:0.75rem; letter-spacing:1px;">EXPLORE LEDGER <i class="ph ph-arrow-square-out"></i></a>`;
        setTimeout(() => bar.classList.remove("visible"), 8000);
    } else if (state === "failed") {
        bar.style.borderColor = "var(--accent)";
        bar.innerHTML = `<i class="ph-bold ph-warning-circle" style="color:var(--accent);"></i> Neural Link Severed (Error)`;
        setTimeout(() => bar.classList.remove("visible"), 8000);
    }
}
